document.addEventListener("DOMContentLoaded", () => {
    ...
    // var acc = document.querySelectorAll(".burger-modal-content > ul > li a");

    // acc.forEach(function (link) {
    //     link.firstClick = true;
    ...
    var productTitleAcc = document.querySelectorAll(".products__accordion");
    var productSubAcc = document.querySelectorAll(".products__sub-accordion");
    ...
    function toggleAccordion() {
        ...
        for (var i = 0; i < productTitleAcc.length; i++) {
            if (productTitleAcc[i] !== this && productTitleAcc[i].classList.contains("active")) {
                productTitleAcc[i].classList.remove("active");
                togglePanel(productTitleAcc[i].nextElementSibling);
            }
        }
    }
    ...
    function toggleSubAccordion() {
        ...
        var parentPanel = this.closest(".products__panel");
        var currentMaxHeight = parentPanel.style.maxHeight ? parseInt(parentPanel.style.maxHeight) : parentPanel.scrollHeight;

        if (subPanel.style.maxHeight) {
            subPanel.style.maxHeight = null;
            parentPanel.style.maxHeight = `${currentMaxHeight - subPanel.scrollHeight}px`;
        } else {
            subPanel.style.maxHeight = subPanel.scrollHeight + "px";
            parentPanel.style.maxHeight = `${currentMaxHeight + subPanel.scrollHeight}px`;
        }

        // Close other open sub-panels
        var otherSubPanels = parentPanel.querySelectorAll(".products__sub-panel");
        for (var i = 0; i < otherSubPanels.length; i++) {
            if (otherSubPanels[i] !== subPanel) {
                otherSubPanels[i].style.maxHeight = null;
            }
        }
    }

    for (var i = 0; i < productTitleAcc.length; i++) {
        productTitleAcc[i].addEventListener("click", toggleAccordion);
    }

    for (var j = 0; j < productSubAcc.length; j++) {
        productSubAcc[j].addEventListener("click", toggleSubAccordion);
    }

    var qualityStandardsProductsVariantsSwiper = new Swiper(".quality-standards__products-variants-swiper", {
        slidesPerView: 4,
        centeredSlides: false,
        ...
        scrollbar: {
            el: ".quality-standards__swiper-scrollbar",
        },
        navigation: {
            nextEl: ".quality-standards__swiper-button-next",
            prevEl: ".quality-standards__swiper-button-prev",
        },
    });

    let careerTabsButtons = document.querySelectorAll(".career-tabs__button-wrapper button")
    let careerTabsHelper = document.querySelector('.career-tabs__helper')
    
    careerTabsButtons.forEach(e => {
        e.addEventListener('click', el => {
            if(e === careerTabsButtons[0]) {
                careerTabsHelper.classList.remove('background')
                careerTabsButtons[0].classList.remove('background')
                careerTabsButtons[1].classList.remove('background')
            }else if (e === careerTabsButtons[1]) {
                careerTabsHelper.classList.add('background')
                careerTabsButtons[0].classList.add('background')
                careerTabsButtons[1].classList.add('background')
            }

            careerTabsHelper.innerHTML = e.nextElementSibling.innerHTML
